...
# To minimize obtrusiviness of animations for the sake of easy visual location (nistagmus prevention; friendliness to 3d‑blind women) —in physical world upon encountering black object on black&white background i either try moving it either try scanning the environment (protruding knife @ big bowl + white plate -- small black cup)
...
# Zooming interface for mode-switching and editing commands
...
# A versioning with an approvement prompt instead of the overwrite
...
# Unlimited treefingers-style undo/redo (for all versions naturally)
...
# To account for memory+sensory troubles: 'if i do not see water then i cannot choose between two bowls using a placement of lid [green plate] (even if the target stands on a tray [which is white plate and is on pale sheet])'; also —'pop‑soda' —'in routine feeding taking a pill from a blister is unmemorisable if there are other supplements in a batch'; also —'blurring of blurred' —i cannot see even textured noncolored glass on a spotted or blotched surface
